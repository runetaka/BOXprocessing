...
protocol SearchTableViewDelegate{
    func searchAlgolia(searchQuery:SearchQuery,isFirst:Bool)
...
class SearchTableViewController: UITableViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBOutlet weak var minIncomeTextField: UITextField!
    @IBOutlet weak var maxIncomeTextField: UITextField!
    ...
    @IBOutlet weak var minIncomeLabel: UILabel!
    @IBOutlet weak var maxIncomeLabel: UILabel!
    
    @IBOutlet weak var hashTagView: UIView!
    @IBOutlet weak var hashTagLabel: UILabel!
    @IBOutlet weak var hashTagViewHeight: NSLayoutConstraint!
    @IBOutlet weak var searchButton: UIButton!
    
    
    
    let sexArray = ["未設定","男性","女性","男女"]
    let searchList = ["フリーワード","勤務地","職種","年収"]
    
    
    
    let prefectures = [ "未設定","北海道", "青森県", "岩手県", "宮城県", "秋田県",
                                          "山形県", "福島県", "茨城県", "栃木県", "群馬県",
    ...
    let professions = ["電気工事","管工事","土木工事"]
    let detailProfessions = [["プラント設備","給排水・空調・ガス配管"],[]]
    ...
    ["岐阜市","大垣市","高山市","多治見市","関市","中津川市","美濃市","瑞浪市","羽島市","恵那市","美濃加茂市","土岐市","各務原市","可児市","山市","瑞穂市","飛騨市","本巣市","郡上市","下呂市","海津市","岐南町","笠松町","養老町","垂井町","関ケ原町","神戸町","輪之内町","安八町","揖斐川町","大野町","池田町","北方町","坂祝町","富加町","川辺町","七宗町","八百津町","白川町","東白川村","御嵩町","白川村"],
    ...
    let incomes = ["未設定","100万円","200万","300万円","400万円","500万円","600万円","700万円","800万円","900万円","1000万円","1000万円以上"]
    let incomesInt = [0,100,200,300,400,500,600,700,800,900,1000,1001]
    
    //入力ずみの検索情報
    var searchArea : String?
    var searchProfession : String?
    var searchMinIncome : Int?
    var searchMaxIncome : Int?
    var searchHoliday : String?
    
    
    var areaPicker = UIPickerView()
    var professionPicker = UIPickerView()
    var minIncomePicker = UIPickerView()
//    var maxIncomePicker = UIPickerView()
...
//    var mode : Mode = .searchGroup
    
    var delegate:SearchTableViewDelegate?
    var beforeQuery:SearchQuery?
    
    
    override func viewDidLoad() {
        super.viewDidLoad()
        labels = [UILabel(),areaLabel,minIncomeLabel,maxIncomeLabel]
        
        pickers = [areaPicker,professionPicker,minIncomePicker]
        setupPickerView()
//        setupTextField()
        ...
        minIncomeTextField.isHidden = true
        maxIncomeTextField.isHidden = true
    ...
    override func viewDidLayoutSubviews() {
        ...
        if let beforeQuery = beforeQuery,let hashTags = beforeQuery.hashTags {
            //検索中の条件がある場合
        ...
        if !(selectedTags.isEmpty){
            if !(tagIsAdded){
                tagIsAdded = true
                addTag(strings: selectedTags, color: .init(hex: "ed7864"), view: hashTagView)
            }
            self.hashTagLabel.isHidden = true
//            tagCollection.tags = []
//        for tag in selectedTags{
//            let name = "#" + tag
//            tagCollection.addNewTag(named: name)
//        }
        }else{
            self.hashTagLabel.isHidden = false
    ...
    func setupNavigationTitle(){
        ...
        let dismissButton = UIBarButtonItem(image: UIImage(systemName: "multiply"), style: .plain, target: self, action: #selector(tappedDismissButton))
...
//        searchButton.layer.shadowOffset = .zero
        self.navigationItem.leftBarButtonItems = [dismissButton,titleItem]
    }
 
    
    func addTag(strings:[String],color:UIColor,view:UIView){
        //staticTableCell用,複数行,長いlabelを飛ばさない
        var i :CGFloat = 0
        var row : CGFloat = 1
        var fullLength :CGFloat = 0.0
        view.layoutIfNeeded()
        let viewWidth = view.frame.width == 0 ? 80 : view.frame.width
        ...
        for string in strings {
            ...
            let height = label.intrinsicContentSize.height + 6
            
            if viewWidth < i * 10 + fullLength + width{
                print(string)
                i = 0
                fullLength = 0
                row += 1
            ...
            label.frame = CGRect(x: i * 10 + fullLength, y:  5 * row + height * (row - 1) , width: width, height: height)
            label.cornerRadius = label.frame.height / 2
            view.addSubview(label)
            self.hashTagViewHeight.constant = label.frame.maxY + 10
            fullLength += width
            i += 1
        }
...
//    func setupTextField(){
//        areaTextField.isHidden = true
////        areaTextField.setUnderLine()
////        minAgeTextField.setUnderLine()
////        maxAgeTextField.setUnderLine()
////        memberNumTextField.setUnderLine()
//
//        if let sex = beforeQuery?.sex{
//            switch sex{
//            case "男女":
//                filters?[0] = "男女"
...
//        }
//
//        if let area = beforeQuery?.area{
//            filters?[1] = area
//            areaTextField.text = area
//            areaLabel.text = area
//            areaLabel.textColor = .label
//            if let index = prefectures.firstIndex(where: {$0 == area}){
//            areaPicker.selectRow(index, inComponent: 0, animated: false)
//            }
//
//        }else{
//            areaTextField.placeholder = "未選択"
//            areaLabel.textColor = .placeholderText
//
//        }
//        if let minAge = beforeQuery?.minAge{
//            filters?[2] = minAge
//
//            minAgePicker.selectRow(minAge - 17, inComponent: 0, animated: false)
//
//            minAgeTextField.text = String(describing: minAge)
//            minAgeLabel.text = "\(minAge)歳"
//            minAgeLabel.textColor = .label
//        }else{
//            minAgeTextField.placeholder = "18"
//            minAgeLabel.text = "未設定"
//            minAgeLabel.textColor = .placeholderText
//        }
//        if let maxAge = beforeQuery?.maxAge{
//            filters?[3] = maxAge
//            minAgePicker.selectRow(maxAge - 17, inComponent: 1, animated: false)
//
//            maxAgeTextField.text = String(describing: maxAge)
//            maxAgeLabel.text = "\(maxAge)歳"
//            maxAgeLabel.textColor = .label
//        }else{
//            maxAgeTextField.placeholder = "70"
//            maxAgeLabel.text = "未設定"
//            maxAgeLabel.textColor = .placeholderText
//        }
//        if let memberNum = beforeQuery?.memberNum{
//            filters?[4] = memberNum
//            memberNumTextField.text = String(describing: memberNum)
//            numberOfMemberLabel.text = "\(memberNum)人"
//            numberOfMemberLabel.textColor = .label
//            memberNumPicker.selectRow(memberNum - 1, inComponent: 0, animated: false)
//        }else{
//            memberNumTextField.placeholder = "未選択"
//            numberOfMemberLabel.text = "未設定"
//            numberOfMemberLabel.textColor = .placeholderText
//        }
//    }
    ...
    func setupPickerView(){
        ...
        minIncomePicker.delegate = self
        minIncomePicker.dataSource = self
        minIncomePicker.tag = 3
        let label = UILabel()
        label.text = "~"
        minIncomePicker.setLabelAtCenter(label: label)

        
//        maxIncomePicker.delegate = self
//        maxIncomePicker.dataSource = self
//        maxIncomePicker.tag = 4
//
     

        toolbar = UIToolbar(frame: CGRect(x: 0, y: 0, width: view.frame.width, height: 45))
...
//
        ...
        minIncomeTextField.inputView = minIncomePicker
        minIncomeTextField.inputAccessoryView = toolbar
        
//        maxIncomeTextField.inputView = maxIncomePicker
//        maxIncomeTextField.inputAccessoryView = toolbar
        
//        professionTextField.inputView = professionPicker
//        professionTextField.inputAccessoryView = toolbar
    ...
    @objc func done(){
        if let selectIndex = selectIndex {
            //開いていたピッカーを取得
//            let searchList = ["勤務地","職種","年収","休日・働き方"]
            
            //textFieldで選択中の内容をlabelと変数に適用

            switch(selectIndex){
            case 1:
                print("勤務地")
                let selectedRow = self.areaPicker.selectedRow(inComponent: 0)
                let area = prefectures[selectedRow]
            ...
            case 2:
                print("職種")
                let selectedRow = self.professionPicker.selectedRow(inComponent: 0)
                let option = professions[selectedRow]
                professionLabel.textColor = .label

                if option == "未設定" || option == ""{
                    
                    searchProfession = nil
                    professionLabel.text = "未設定"
                    professionLabel.textColor = .placeholderText
                    return
                }else{
                    professionLabel.textColor = .label
                    searchProfession = option
                    professionLabel.text = option
                    
                }
            case 3:
                print("年収")
                
                let selectedRow0 = self.minIncomePicker.selectedRow(inComponent: 0)
                let selectedRow1 = self.minIncomePicker.selectedRow(inComponent: 1)
                let minIncome = incomes[selectedRow0]
                let maxIncome = incomes[selectedRow1]
                
                
//                let minIncome = minIncomeTextField.text ?? "未設定"
//                let maxIncome = maxIncomeTextField.text ?? "未設定"
                
                
                //両方が未設定でない場合のみ並び替え
                if minIncome != "未設定" && maxIncome != "未設定"{
                    
                    if let minIndex = incomes.firstIndex(of: minIncome),let maxIndex = incomes.firstIndex(of: maxIncome){
                        let minInt = incomesInt[minIndex]
                        let maxInt = incomesInt[maxIndex]
                        if minInt < maxInt{
                            self.searchMinIncome = minInt
                            self.searchMaxIncome = maxInt
                            self.minIncomeLabel.text = minIncome
                            self.maxIncomeLabel.text = maxIncome
                            self.minIncomeLabel.textColor = .label
                            self.maxIncomeLabel.textColor = .label

                        }else{
                            self.searchMinIncome = maxInt
                            self.searchMaxIncome = minInt
                            self.minIncomeLabel.text = maxIncome
                            self.maxIncomeLabel.text = minIncome
                            self.minIncomeLabel.textColor = .label
                            self.maxIncomeLabel.textColor = .label
                    ...
                    if let minIndex = incomes.firstIndex(of: minIncome),let maxIndex = incomes.firstIndex(of: maxIncome){
                        ...
                        if minIncome == "未設定"{
                            self.minIncomeLabel.textColor = .placeholderText
                            self.searchMinIncome = nil
                        }else{
                            self.minIncomeLabel.textColor = .label
                            self.searchMinIncome = minInt
                        }
                        ...
                        if maxIncome == "未設定"{
                            self.maxIncomeLabel.textColor = .placeholderText
                            self.searchMaxIncome = nil
                        }else{
                            self.maxIncomeLabel.textColor = .label
                            self.searchMaxIncome = maxInt
                        }
...
//
        ...
        minIncomeTextField.endEditing(true)
        maxIncomeTextField.endEditing(true)
        areaTextField.endEditing(true)
        professionTextField.endEditing(true)
    }
    @objc func cancel(_ sender:UIGestureRecognizer){
        
        //        let searchList = ["フリーワード","勤務地","職種","年収","休日・働き方"]

        guard let selectIndex = selectIndex else {return}
        if selectIndex == 1{
            guard let currentValue  = areaLabel.text else{return}
            if  currentValue != "未設定"{
                self.areaTextField.text = self.areaLabel.text
            ...
            //職種
            //cuurentValue : 決定済みのvalue
            guard let currentValue = self.professionTextField.text else{return}
            if  currentValue != "未設定"{
                self.professionTextField.text = self.professionLabel.text
                if let row = professions.firstIndex(where: {$0 == "\(currentValue)"}){
                    professionPicker.selectRow(row, inComponent: 0, animated: false)
                }
            }else{
                self.professionTextField.text = self.professionLabel.text
                professionPicker.selectRow(0, inComponent: 0, animated: false)
            }
            ...
            //働き方?
            guard let currentValue  = self.labels[selectIndex + 1].text else{return}
            if  currentValue != "未設定"{
            self.textfields[selectIndex + 1].text = self.labels[selectIndex + 1].text
                if let row = searchArray[selectIndex].firstIndex(where: {$0 == "\(currentValue)"}){
                    pickers[selectIndex - 1].selectRow(row, inComponent: 0, animated: false)
                }
            }else{
                self.textfields[selectIndex + 1].text = self.labels[selectIndex + 1].text
                pickers[selectIndex - 1].selectRow(0, inComponent: 0, animated: false)

            }
        }
        self.selectIndex = nil
        print("search",self.filters?[selectIndex + 1])
        minIncomeTextField.endEditing(true)
        maxIncomeTextField.endEditing(true)
        areaTextField.endEditing(true)
//        professionTextField.endEditing(true)
    ...
    @IBAction func tappedSearchButton(_ sender: Any) {
        ...
        let dic = ["sex":filters?[0],
                   "area":filters?[1],
                   "minAge":filters?[2],
                   "maxAge":filters?[3],
                   "memberNum":filters?[4]] as [String : Any]
        
        let query = SearchQuery(dic: dic)
        
        query.hashTags = selectedTags
        print("query",dic)
        
        if query.maxAge == nil && query.memberNum == nil && query.minAge == nil && selectedTags.isEmpty{
                self.delegate?.search(query: query)
            }else{
                self.delegate?.searchAlgolia(searchQuery: query,isFirst : true)
            }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let selectIndex = selectIndex,selectIndex != indexPath.row,selectIndex != 0 {
            //前に選択していたpickerをdone
            self.done()
        }
...
//        cell?.selectionStyle = .none
        ...
        let cancelItem = UIBarButtonItem(title: "キャンセル", style: .done, target: .none, action: #selector(cancel(_:)))
        let label = UILabel()
        label.text = self.searchList[indexPath.row]
        let titleItem = UIBarButtonItem(customView: label)
        let spaceItem = UIBarButtonItem(barButtonSystemItem: .flexibleSpace, target: nil, action: .none)
        let doneItem = UIBarButtonItem(title: "決定", style: .done, target: self,action: #selector(done))
        self.toolbar.setItems([cancelItem,spaceItem,titleItem,spaceItem,doneItem], animated: false)
        self.selectIndex = indexPath.row
            }
        switch indexPath.row{
        case 0:
            print("freeWord")
//            let vc = self.storyboard?.instantiateViewController(withIdentifier: "SelectPurposeTableViewController") as! SelectPurposeTableViewController
//            vc.selectedTags = selectedTags
            let vc = QuerySuggestionsDemoViewController()
            self.navigationController?.pushViewController(vc, animated: true)
        case 1:
            //勤務地
            
            if let selectedPrefecture = searchArea,let selectedRow = prefectures.firstIndex(of: selectedPrefecture){
                areaPicker.selectRow(selectedRow, inComponent: 0, animated: false)
            }
            areaTextField.inputAccessoryView = toolbar
        ...
        case 3:
            ...
            if let selectedMin = searchMinIncome,let selectedRow0 = incomesInt.firstIndex(of: selectedMin){
                minIncomePicker.selectRow(selectedRow0, inComponent: 0, animated: false)
            }
            if let selectedMax = searchMaxIncome,let selectedRow1 = incomesInt.firstIndex(of: selectedMax){
                minIncomePicker.selectRow(selectedRow1, inComponent: 1, animated: false)
            }
            
            minIncomeTextField.becomeFirstResponder()
            minIncomeTextField.inputAccessoryView = toolbar
        case 4:
            print("holiday")
...
//        let searchList = ["フリーワード","勤務地","職種","年収","休日・働き方"]
    ...
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        ...
        if pickerView.tag == 1{
           ...
           return professions[row]
            ...
            //年収
            if minIncomeTextField.text == ""{
                if row < incomesInt.count{
                    minIncomeTextField.text = incomes[row]
                    self.searchMinIncome = incomesInt[row]
                }else{
                    minIncomeTextField.text = "未設定"
                }
            }
            if maxIncomeTextField.text == ""{
                if row < incomesInt.count{
                    maxIncomeTextField.text = incomes[row]
                    self.searchMaxIncome = incomesInt[row]
                }else{
                    maxIncomeTextField.text = "未設定"
                }
            }
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        if pickerView.tag == 1{
            //勤務地
            areaTextField.text = prefectures[row]
            
        }else if pickerView.tag == 2{
            //職種
            professionTextField.text = professions[row]
        }else{
            //年収
            switch component{
            case 0:
